...
単なる記録アプリにとどまらず、夫婦間のコミュニケーションをより円滑にするための機能拡張を予定しています。

### 🔄 1. 夫婦アカウントの「ペアリング機能」の実装
現在は夫側による「観察・記録」が主ですが、妻側もアカウントを持ち、相互にデータを共有できる機能を実装予定です。
*   **目的:** 妻自身が体調や気分別を入力することで、AIの推測ではなく「正確な一次データ」に基づいたアドバイスを可能にするため。
*   **技術的挑戦:** `User` テーブル同士の紐付け（多対多、または1対1のリレーション構築）や、相互のデータ閲覧権限を管理する認可ポリシの実装に取り組みます。

### 📈 2. 長期データの分析・レポート機能
蓄積されたログデータを分析し、傾向を可視化する機能を実装予定です。
*   **目的:** 「月曜日の朝は機嫌が崩れやすい」「生理前3日はスコアが低下する」といった長期的なバイオリズムを可視化し、トラブルを未然に回避する「予防」に役立てるため。
*   **技術的挑戦:** SQLによる期間別データの集計処理や、JavaScriptライブラリ（Chart.jsなど）を用いた動的なグラフ描画の実装を想定しています。